<template>
  <div class="el-main">
    ...
    <el-table :data="showList" style="width: 100%;" height="470px">
      ...
      <el-table-column prop="teacher.nickname" label="指导教师"></el-table-column>
      ...
      <el-table-column label="操作" align="center" width="200">
        <template slot-scope="scope">
          <el-button size="small" v-if="scope.row.status == 3" @click="showReason(scope.row.id)">查看理由</el-button>
          <el-button size="small" type="success" v-if="scope.row.status == 3 || scope.row.status == 0" @click="update(scope.row.id)">修改</el-button>
          <el-button size="small" type="danger" v-if=" scope.row.status == 0" @click="del(scope.row.id)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
...
<script>
import AddTeacherActivityDialog from './dialog/AddClubActivityDialog.vue'
...
export default {
  ...
  methods: {
    showReason(id){
      this.$axios.get("apply/getReason?id="+id).then(res=>{
        this.$alert(res.data.data.reason)
      })
    },
    ...
    loadList(){
      this.$axios.get("/activity/club?cid="+this.cid).then(res=>{
        this.dataList = res.data.data;
        this.page.total = this.dataList.length;
        this.currentChange(1)
      })
  ...
  mounted() {
    this.cid = JSON.parse(sessionStorage.getItem("user")).id
